...
export default function ResourcePersonIdGenerationPage() {
  ...
  useEffect(() => {
    const loadGeneratedIds = async () => {
      try {
        setIsLoading(true);
        const ids = await getGeneratedIds('resource_person');
        setGeneratedIds(ids);
      } catch (error) {
  ...
  const generateResourcePersonId = async () => {
    try {
      ...
      setError('Failed to generate resource person ID');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Resource Person ID Management</h2>
          <p className="text-gray-600">Generate and manage resource person identification numbers with their details.</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="p-6 border-b border-gray-200">
            <div className="flex justify-between items-center">
              ...
              <Button 
                onClick={generateResourcePersonId}
                disabled={isGenerating}
                className="bg-amber-600 hover:bg-amber-700"
              >
                <Plus className="mr-2" size={16} />
                {isGenerating ? 'Generating...' : 'Generate Resource Person ID'}
              </Button>
            </div>
          </div>
                    ...
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                        <div className="flex flex-col items-center">
                          ...
                          <p className="text-gray-500 mb-4">No resource person IDs have been generated yet.</p>
                          <Button 
                            onClick={generateResourcePersonId}
                            disabled={isGenerating}
                            className="bg-amber-600 hover:bg-amber-700"
